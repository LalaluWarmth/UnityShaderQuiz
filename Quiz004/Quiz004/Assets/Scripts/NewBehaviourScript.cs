...
public class NewBehaviorScript : MonoBehaviour
{
    ...
    private void OnWillRenderObject()
    {
        ...
        var inDir = transform.position - Camera.main.transform.position;
        reflectionCamera.transform.position = transform.position - Vector3.Reflect(inDir, transform.up);
        reflectionCamera.transform.rotation = Quaternion.LookRotation(transform.up, transform.forward);

        var pos = transform.worldToLocalMatrix * new Vector4(reflectionCamera.transform.position.x,
            reflectionCamera.transform.position.y, reflectionCamera.transform.position.z, 1);
        var size = new Vector2(transform.localScale.x, transform.localScale.z);
        reflectionCamera.sensorSize = size;
        reflectionCamera.nearClipPlane = Mathf.Abs(pos.y);
        reflectionCamera.fieldOfView = Mathf.Atan(size.y / 2 / Mathf.Abs(pos.y)) * Mathf.Rad2Deg * 2;
        reflectionCamera.lensShift = new Vector2(pos.x, -pos.z);
