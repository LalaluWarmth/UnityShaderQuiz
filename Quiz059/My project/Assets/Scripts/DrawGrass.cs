...
public class DrawGrass : MonoBehaviour
{
    ...
    public void Draw()
    {
        ...
        SetupGrassBuffers();
        maxHeight += 1.5f;
        grassBounds = new Bounds(new Vector3(selfPosition.x, maxHeight / 2, selfPosition.z),
            new Vector3(xRange * 2, maxHeight / 2, zRange * 2));
    }
    ...
    void SetupGrassBuffers()
    {
        ...
        for (int i = 0; i < grassCount; i++)
        {
            ...
            if (GetGround(ref randomPos))
            {
                ...
                matrixList.Add(Matrix4x4.TRS(randomPos, Quaternion.Euler(0F, rotateY, 0F),
                    new Vector3(1, heightScale, 1)));
            }
        }

        grassMaterix4X4 = matrixList.ToArray();
        Debug.Log(grassMaterix4X4);
    }
